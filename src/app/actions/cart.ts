...
export async function addToCart(formData: FormData) {
  ...
  let cart: CartItem[] = [];
  try { cart = JSON.parse(cartCookie); } catch { cart = []; }

  const idx = cart.findIndex(c => c.movieId === movieId);
  if (idx >= 0) cart[idx].quantity += 1;
  else cart.push({ movieId, quantity: 1 });

  if (typeof cs.set === 'function') cs.set('cart', JSON.stringify(cart), { path: '/' });
  revalidatePath('/cart');
}
...
export async function updateCart(formData: FormData) {
  ...
  const cartCookie = typeof cs2.get === 'function' ? cs2.get('cart')?.value || '[]' : '[]';
  let cart: CartItem[] = [];
  try { cart = JSON.parse(cartCookie); } catch { cart = []; }
  ...
  if (typeof cs2.set === 'function') cs2.set('cart', JSON.stringify(cart), { path: '/' });
  revalidatePath('/cart');
}
